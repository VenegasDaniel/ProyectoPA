	...
	public Login(ConnectionDB connect) {
		...
		login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				rut = validateRut(rut);
				if(rut != null) {
					String query = String.format("Select * from Person where rut = '%s' or ruthCompany = '%s' ",rut,rut);
					try {
						...
						if(l.isEmpty()) {
							...
							if(l.get(9) != null) {
								if(l.get(9).equals(rut) && l.get(9).equals(passwordPerson)) {
									Person p = Factory.factory( l.get(0),l.get(1),l.get(2),l.get(3), l.get(4),  l.get(5),  l.get(6),  l.get(7),  l.get(8),  l.get(9),  l.get(10));
									//AdminScreen as = new AdminScreen(p,connect);
									//as.setVisible(true);
									Login.this.dispose();
								}
								else {
